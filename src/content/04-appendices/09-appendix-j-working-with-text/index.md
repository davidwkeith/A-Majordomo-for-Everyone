---
title: "Appendix J: Working With Text — How Your Agent Reads"
part: 4
order: 9
strategy: null
status: "draft"
...
When you open a conversation with your Agent, you are opening a shared workspace. Everything in that workspace — your messages, the Agent's replies, any documents you paste in — occupies space in the Agent's [working memory](https://en.wikipedia.org/wiki/Working_memory). The technical term is the *context window*. Think of it as a desk.
...
Your Agent's working memory is large — tens of thousands of words in a single conversation. But it is not infinite, and not all of it receives equal attention. The beginning and end of a conversation are processed most reliably. The middle can get murky in long sessions. This is not a flaw. It is how the technology works, and knowing it changes how you use it.
...
You do not need to count tokens. You need to know one thing: **everything you put in the conversation uses some of your budget, including the parts you did not mean to include.** The signature block at the bottom of a forwarded email. The legal disclaimer on a bank statement. The forty-seven previous replies in a forwarded email chain. All of it costs tokens and none of it helps your Agent think about your question.
...
**Text is cheap.** A full page of text — about 500 words — costs roughly 650 tokens. Your Agent can read it precisely, search within it, and quote it back to you.

**Images are expensive.** A photograph of that same page costs roughly 1,500 tokens — more than double — and the Agent has to interpret the image before it can work with the content. Handwriting, poor lighting, glare, and low resolution all degrade accuracy.
...
This is the majordomo principle in reverse. The billionaire's chief of staff did not hand the boss a filing cabinet. She read the filing cabinet, extracted the relevant page, highlighted the relevant paragraph, and handed over one sheet of paper. You can do the same thing for your Agent — or you can ask your Agent to do it for you.

**Don't paste the whole document when one section will do.** A 40-page lease has one clause you're confused about. Paste that clause, plus the definitions section if there is one. Your Agent will give you a better answer from two focused paragraphs than from forty pages of boilerplate.

**Don't include the email chain when you need the decision.** If someone forwarded you a thread with twenty replies, paste only the messages that contain the actual information. Or paste the whole thread and say: *"Summarize only the decisions that were made and the open questions that remain."*

**Start fresh when the conversation gets long.** If you have been going back and forth for thirty messages and the Agent starts repeating itself, losing track of earlier points, or giving vague answers — it is not broken. Its desk is full. Start a new conversation. Paste in only the conclusions from the last one. This is not starting over. It is giving your Agent a clean briefing.
...
These tips appear throughout the Field Guide in the entries where you will actually encounter them. Here they are in one place for reference.
